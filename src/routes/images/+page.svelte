<script lang="ts">
  ...
  const captures = capturesByType('image');
  ...
  async function handleSave(data: CreateCaptureInput) {
    ...
    await addCapture({ ...data, type: 'image' });
  }
...
<svelte:head><title>Images - Motif</title></svelte:head>

<div class="page fade-in">
  <div class="page-header">
    <div class="header-left">
      <h1 class="page-title"><span class="title-icon"><NavIcon name="image" size={20} /></span>{t('nav.images')}</h1>
      <span class="count">{$captures.length}</span>
    </div>
    <button class="btn-new" onclick={openNewCapture}>+ {t('capture.addImage')}</button>
  </div>
  ...
  {:else}
    <div class="empty slide-up">
      <div class="empty-icon"><NavIcon name="image" size={44} strokeWidth={1.6} /></div>
      <h2>{t('empty.images')}</h2>
      <p>{t('empty.imagesHint')}</p>
    </div>
  {/if}
</div>

<CaptureModal bind:open={showModal} defaultTab="image" onSave={handleSave} initialData={initialModalData} />
