<script lang="ts">
  ...
  async function handleSave(data: CreateCaptureInput) { await addCapture({ ...data, type: 'image' }); }
...
<div class="page fade-in">
  <div class="page-header">
    <div class="header-left">
      <h1 class="page-title">◻ {t('nav.images')}</h1>
      <span class="count">{$captures.length}</span>
    </div>
    <button class="btn-new" onclick={() => showModal = true}>+ {t('capture.addImage')}</button>
  </div>
...
<CaptureModal bind:open={showModal} defaultTab="image" onSave={handleSave} />
