<script lang="ts">
  ...
  async function handleSave(data: CreateCaptureInput) {
    ...
    await addCapture({ ...data, type: 'quote' });
  }
...
<div class="page fade-in">
  <div class="page-header">
    <div class="header-left">
      <h1 class="page-title"><span class="title-icon"><NavIcon name="quote" size={20} /></span>{t('nav.quotes')}</h1>
      <span class="count">{$captures.length}</span>
    </div>
    <button class="btn-new" onclick={openNewCapture}>+ {t('capture.addQuote')}</button>
  </div>
  ...
  {:else}
    <div class="empty slide-up">
      <div class="empty-icon"><NavIcon name="quote" size={44} strokeWidth={1.6} /></div>
...
<CaptureModal bind:open={showModal} defaultTab="quote" onSave={handleSave} initialData={initialModalData} />
