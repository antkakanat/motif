<script lang="ts">
  ...
  async function handleSave(data: CreateCaptureInput) {
    ...
    await addCapture({ ...data, type: 'link' });
  }
...
<svelte:head><title>Links - Motif</title></svelte:head>

<div class="page fade-in">
  <div class="page-header">
    <div class="header-left">
      <h1 class="page-title"><span class="title-icon"><NavIcon name="link" size={20} /></span>{t('nav.links')}</h1>
      <span class="count">{$captures.length}</span>
    </div>
    <button class="btn-new" onclick={openNewCapture}>+ {t('capture.addLink')}</button>
  </div>
  ...
  {:else}
    <div class="empty slide-up">
      <div class="empty-icon"><NavIcon name="link" size={44} strokeWidth={1.6} /></div>
      <h2>{t('empty.links')}</h2>
      <p>{t('empty.linksHint')}</p>
    </div>
  {/if}
</div>

<CaptureModal bind:open={showModal} defaultTab="link" onSave={handleSave} initialData={initialModalData} />
